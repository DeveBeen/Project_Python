...
while True:
    ...
    if program_swich1 == 0:
        ...
        print('You are fined ester-egg!! ')

    else:
        test_num = word_extract(add_day(day_list)) # 단어 가져오기

        question_count = int(input('Please select you want number of question.'))
        question_answer = answer_random_set(question_count,test_num) # 함수가 연속으로 실행되면 값이 난수 값으로 계속 변경되므로 변수에 각각 할당한다.
        question_type = type_random_set(question_count)
        question_correct = correct_random_set(question_count)
        question_example = example_random_set_list(question_count, question_answer, test_num)
